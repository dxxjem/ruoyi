...
基于 RuoYi-Vue-Plus v5.5.1 框架构建的 B/S 架构智能合规审计平台，采用"规则引擎 + 关键词匹配"技术，实现 PDF 文档的条款级合规性自动检查。
...
└── src/main/java/org/dromara/compliance/
    ├── config/
    │   └── PostgreSQLVectorConfig.java  ← PostgreSQL多数据源配置
    ├── controller/
    ...
    │   ├── IDocumentParserService.java  ← PDF解析服务
    │   ├── IVectorEmbeddingService.java  ← 向量生成服务
    │   ├── IComplianceCheckService.java  ← 检查核心服务
    │   └── impl/
    └── snailjob/
        └── ComplianceCheckJobExecutor.java  ← SnailJob执行器
```
...
**任务3.3**: 文档管理功能
- 创建ComplianceDocument相关类
- 实现文档上传和列表
- 集成OSS文件存储

**验收标准**: 可以上传并解析PDF，可以生成向量
...
**任务4.2**: 任务管理功能
- 创建ComplianceCheckTask相关类
- 实现任务创建和查询
- 实现任务状态更新

**任务4.3**: 检查结果存储
- 创建ComplianceCheckResult相关类
- 实现结果保存逻辑
- 实现结果查询接口
...
**任务5.3**: Excel导出功能
- 创建ComplianceCheckResultExportVo
- 使用ExcelUtil实现导出
- 前端导出按钮
...
2. `D:\test\ruoyi\RuoYi-Vue-Plus-v5.5.1\ruoyi-modules\ruoyi-compliance\src\main\java\org\dromara\compliance\config\PostgreSQLVectorConfig.java`
   - PostgreSQL多数据源配置

3. `D:\test\ruoyi\RuoYi-Vue-Plus-v5.5.1\ruoyi-modules\ruoyi-compliance\src\main\java\org\dromara\compliance\service\impl\ComplianceCheckServiceImpl.java`
   - 硬规则检查核心逻辑

4. `D:\test\ruoyi\RuoYi-Vue-Plus-v5.5.1\ruoyi-modules\ruoyi-compliance\src\main\java\org\dromara\compliance\snailjob\ComplianceCheckJobExecutor.java`
   - SnailJob异步任务执行器
...
7. `D:\test\ruoyi\plus-ui-v5.5.1-v2.5.1\src\views\compliance\document\index.vue`
   - 文档上传和任务创建页面

8. `D:\test\ruoyi\plus-ui-v5.5.1-v2.5.1\src\api\compliance\task\index.ts`
   - 任务管理API定义

### 配置文件

9. `D:\test\ruoyi\RuoYi-Vue-Plus-v5.5.1\ruoyi-admin\src\main\resources\application-dev.yml`
   - 添加PostgreSQL向量数据源配置

10. `D:\test\ruoyi\RuoYi-Vue-Plus-v5.5.1\ruoyi-admin\pom.xml`
   - 添加compliance模块依赖
...
#### 查询检查结果列表
```
GET /compliance/check/result/list?taskId=100
```

#### 导出Excel
```
GET /compliance/report/export/{taskId}
```
